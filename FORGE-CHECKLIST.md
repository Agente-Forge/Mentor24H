...
| 2026-05-16 | Sidebar Premium v5.2 — Visual | **skill-forge-visual v5.0 (PAE #5 · Etapa 2):** CSS premium das 5 zonas semânticas. **css/sidebar.css (novo):** 9 seções, ~340 linhas. Z1: padding-top 28px, separador gradiente `::after`. Z2: avatar ring `box-shadow 0 0 0 2px var(--color-gold)`, animação `sb-avatar-pulse` 400ms (ouro/safira por modo), `#sidebar-saudacao` Fraunces italic. Z3: switcher frosted glass (`@supports backdrop-filter`), pill ouro com glow shadow, pill safira em negócio, rail compacto (ícones empilhados em coluna). Z4: dot indicator 6×18px com glow luminoso, animação `sb-dot-in` ease-spring, frosted glass hover, safira em modo negócio. Z5: gradient separator `::before`, Chat IA destacado (gradiente ouro/safira + hover translateX+glow), collapse btn premium (surface-2 border-1). Rail mode: `#sidebar.collapsed` com `overflow: visible` para tooltips CSS `attr(data-tooltip)` com delay 200ms. Mobile drawer: `.mobile-open` restaura padding das zonas. Light theme: ring e gradientes com tokens escuros. Acessibilidade: focus rings 2px ouro/safira + box-shadow halo, `prefers-reduced-motion`. **index.html:** link `sidebar.css?v=1` adicionado, `data-tooltip="..."` em 17 nav items/group-headers. **Zero conflitos** com `layout.css` — todos os estilos são additive. |
