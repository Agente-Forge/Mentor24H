/* ═══════════════════════════════════════════════════════════
   ICONS — Inline SVG system (zero CDN dependency)
   Lucide-inspired, hand-curated for FinFlow
═══════════════════════════════════════════════════════════ */
...
const Icons = (() => {
  const ICONS = {
    ...
    'piggy-bank': '<path d="M19 5c-1.5 0-2.8 1.4-3 2-3.5-1.5-11-.3-11 5 0 1.8 0 3 2 4.5V20h4v-2h3v2h4v-4c1-.5 1.7-1 2-2h2v-4h-2c0-1-.5-1.5-1-2V5z"/><path d="M2 9v1c0 1.1.9 2 2 2h1"/><circle cx="14" cy="10" r="1" fill="currentColor"/>',
    ...
    'leaf': '<path d="M11 20A7 7 0 0 1 9.8 6.1C15.5 5 17 4.48 19.2 2.96c1 6.04 1 11.02-1.5 14.04a8 8 0 0 1-7 4Z"/><path d="M2 21c0-3 1.85-5.36 5.08-6"/>',
    ...
    'address-book': '<path d="M16 2H8a2 2 0 0 0-2 2v16a2 2 0 0 0 2 2h8a2 2 0 0 0 2-2V4a2 2 0 0 0-2-2Z"/><path d="M8 8h8"/><path d="M8 12h8"/><path d="M8 16h4"/><path d="M4 8h2"/><path d="M4 12h2"/><path d="M4 16h2"/>',
  ...
  function svg(name, size = 18, extraClass = '') {
    ...
    if (!path) return '';
    return `<svg xmlns="http://www.w3.org/2000/svg" width="${size}" height="${size}" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="1.75" stroke-linecap="round" stroke-linejoin="round" class="icon ${extraClass}">${path}</svg>`;
  }
  ...
  function render(container = document) {
    container.querySelectorAll('[data-icon]').forEach(el => {
      ...
      const html = svg(name, size);
      if (html) el.innerHTML = html;
    });
  }

  function html(name, size = 18) { return svg(name, size); }
  function exists(name) { return name in ICONS; }
  function list() { return Object.keys(ICONS); }

  return { svg, html, render, exists, list };
})();

window.icon = (name, size) => Icons.html(name, size);
